  ...
  <div class="mb-2 mt-4">
    ...
    <edit-autocomplete class="mb-2">
      <ui-textarea
        ...
        :label="`${t('workflow.blocks.llm.url')}*`"
    ...
    <ui-select
      ...
      :label="t('workflow.blocks.llm.contentType')"
      class="mb-2 w-full"
    ...
    <ui-input
      ...
      :label="
        t('workflow.blocks.llm.timeout.placeholder') +
        ` (${t('common.0disable')})`
      "
      :title="t('workflow.blocks.llm.timeout.title')"
      class="mb-2 w-full"
      type="number"
    ...
    <ui-tabs v-model="activeTab" fill>
      <ui-tab value="headers">
        {{ t('workflow.blocks.llm.tabs.headers') }}
      </ui-tab>
      <ui-tab value="body">
        {{ t('workflow.blocks.llm.tabs.body') }}
      </ui-tab>
      <ui-tab value="response">
        {{ t('workflow.blocks.llm.tabs.response') }}
      </ui-tab>
    </ui-tabs>
    <ui-tab-panels v-model="activeTab">
      ...
      >
        ...
        <ui-button class="col-span-4 mt-4 block w-full" @click="addHeader">
          <span> {{ t('workflow.blocks.llm.buttons.header') }} </span>
        </ui-button>
      </ui-tab-panel>
      <ui-tab-panel value="body" class="mt-4">
        <p class="text-sm  mb-2">Text Context Description</p>
        <ui-textarea
          :model-value="data.textContext"
          :label="t('workflow.blocks.llm.textContext')"
          class="mb-2 w-full"
          rows="3"
          @change="updateData({ textContext: $event })"
        />
        <p class="text-sm  mb-2">Image Context Description</p>
        <ui-textarea
          :model-value="data.imageContext"
          :label="t('workflow.blocks.llm.imageContext')"
          class="mb-2 w-full"
          rows="3"
          @change="updateData({ imageContext: $event })"
        />
        <p class="text-sm mb-2">API Results Description</p>
        <ui-textarea
          :model-value="data.apiResults"
          :label="t('workflow.blocks.llm.apiResults')"
          class="mb-2 w-full"
          rows="3"
          @change="updateData({ apiResults: $event })"
        />
        <p class="text-sm mb-2">User Prompt Description</p>
        <ui-textarea
          :model-value="data.userPrompt"
          :label="t('workflow.blocks.llm.userPrompt')"
          class="mb-2 w-full"
          rows="3"
          @change="updateData({ userPrompt: $event })"
        />
      </ui-tab-panel>
    ...
    <ui-modal
      ...
      :title="t('workflow.blocks.llm.tabs.body')"
    >
      <shared-codemirror
